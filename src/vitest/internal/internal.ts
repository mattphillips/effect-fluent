...
const runPromise =
  ...
  <E, A>(effect: Effect<A, E>) =>
    _Effect
      .gen(function* () {
        const exitFiber = yield* _Effect.fork(_Effect.exit(effect.asEffect));

        ctx?.onTestFinished(() => Fiber.interrupt(exitFiber).pipe(_Effect.asVoid, _Effect.runPromise));
        ...
        if (Exit.isSuccess(exit)) {
          ...
          for (let i = 1; i < errors.length; i++) {
            yield* _Effect.logError(errors[i]);
          }
          return () => {
            throw errors[0];
          };
      ...
      .pipe((effect) => _Effect.runPromise(effect, ctx))
      .then((f) => f());
...
/** @internal */
const makeTester = <R>(
  mapEffect: <A, E>(self: Effect<A, E, R>) => Effect<A, E, never>,
...
export const prop: Vitest.Vitest.Methods['prop'] = (name, arbitraries, self, timeout) => {
  if (Array.isArray(arbitraries)) {
    ...
    return V.it(name, testOptions(timeout), (ctx) =>
      ...
      fc.assert(
        // @ts-ignore
        fc.property(...arbs, (...as) => self(as as any, ctx)),
        isObject(timeout) ? timeout?.fastCheck : {}
      )
    );
  }
...
export const layer =
  <R, E, const ExcludeTestServices extends boolean = false>(
    ...
    (name: string, f: (it: Vitest.Vitest.MethodsNonLive<R, ExcludeTestServices>) => void): void;
  } =>
  (
    ...args:
    ...
    const memoMap = options?.memoMap ?? _Effect.runSync(Layer.makeMemoMap);
    const scope = _Effect.runSync(Scope.make());
    const runtimeEffect = Effect.of(
      Layer.toRuntimeWithMemoMap(withTestEnv, memoMap).pipe(
        Scope.extend(scope),
        _Effect.orDie,
        _Effect.cached,
        _Effect.runSync
    ...
    const makeIt = (it: Vitest.API): Vitest.Vitest.MethodsNonLive<R, ExcludeTestServices> => {
      return Object.assign(it, {
        effect: makeTester<TestServices.TestServices | R>(
          (effect) => runtimeEffect.flatMap((runtime) => effect.asEffect.pipe(_Effect.provide(runtime), Effect.of)),
          it
        ),
        ...
        scoped: makeTester<TestServices.TestServices | Scope.Scope | R>(
          (effect) =>
            runtimeEffect.flatMap((runtime) =>
              effect.asEffect.pipe(_Effect.scoped, _Effect.provide(runtime), Effect.of)
            ),
          it
        ),
        flakyTest,
    ...
    if (args.length === 1) {
      V.beforeAll(
        () => runPromise()(runtimeEffect.asVoid),
...
/** @internal */
export const flakyTest = <A, E, R>(self: Effect<A, E, R>, timeout: Duration.DurationInput = Duration.seconds(30)) =>
  pipe(
    self.asEffect,
    _Effect.catchAllDefect(_Effect.fail),
    _Effect.retry(
      pipe(
        Schedule.recurs(10),
...
export const makeMethods = (it: Vitest.API): Vitest.Vitest.Methods => {
  return Object.assign(it, {
    effect: makeTester<TestServices.TestServices>(
      (effect) => Effect.of(effect.asEffect.pipe(_Effect.provide(TestEnv))),
      it
    ),
    scoped: makeTester<TestServices.TestServices | Scope.Scope>(
      (effect) => effect.scoped.asEffect.pipe(_Effect.provide(TestEnv), Effect.of),
      it
    ),
    live: makeTester<never>(identity, it),
    scopedLive: makeTester<Scope.Scope>((effect) => effect.scoped, it),
    flakyTest,
    layer,
    prop
  });

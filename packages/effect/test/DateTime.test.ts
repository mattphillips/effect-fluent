import { DateTime as _DateTime, Effect as _Effect } from 'effect';
...
const assertSomeIso = (value: Option<DateTime>, expected: string) => {
  const iso = value.map((value) => value.toUtc.formatIso);
...
describe('DateTime', () => {
  describe('mutate', () => {
    it.effect('should mutate the date', () =>
      Effect.gen(function* () {
        ...
        const tomorrow = now.mutate((date) => {
          date.setUTCDate(date.getUTCDate() + 1);
        });
        const diff = now.distance(tomorrow);
    ...
    it.effect('preserves the named zone when mutating across DST', () =>
      Effect.gen(function* () {
        ...
        const future = now.mutate((date) => {
          date.setUTCMonth(date.getUTCMonth() + 6);
        });
        strictEqual(future.toDateUtc.toISOString(), '2024-06-30T12:00:00.000Z');
        strictEqual(future.toDate.toISOString(), '2024-07-01T00:00:00.000Z');
        const plusOne = future.mutate((date) => {
          date.setUTCDate(date.getUTCDate() + 1);
        });
        strictEqual(plusOne.toDateUtc.toISOString(), '2024-07-01T12:00:00.000Z');
        strictEqual(plusOne.toDate.toISOString(), '2024-07-02T00:00:00.000Z');
      }));
  });
  ...
  describe('add', () => {
    ...
    it.effect('preserves the named zone when adding calendar units across DST', () =>
      Effect.gen(function* () {
        yield* setTo2024NZ;
        const now = yield* DateTime.nowInCurrentZone.pipe(DateTime.withCurrentZoneNamed('Pacific/Auckland'));
        const future = now.add({ months: 6 });
        strictEqual(future.toDateUtc.toISOString(), '2024-06-30T12:00:00.000Z');
        strictEqual(future.toDate.toISOString(), '2024-07-01T00:00:00.000Z');
        const plusOne = future.add({ days: 1 });
        strictEqual(plusOne.toDateUtc.toISOString(), '2024-07-01T12:00:00.000Z');
        strictEqual(plusOne.toDate.toISOString(), '2024-07-02T00:00:00.000Z');
        const minusOne = plusOne.subtract({ days: 1 });
        strictEqual(minusOne.toDateUtc.toISOString(), '2024-06-30T12:00:00.000Z');
        strictEqual(minusOne.toDate.toISOString(), '2024-07-01T00:00:00.000Z');
      }));
    ...
    it.effect('leap years', () =>
      Effect.gen(function* () {
        ...
        const future = now.map((value) => value.add({ years: 1 }));
        assertSome(future.map((value) => value.formatIso), '2025-02-28T00:00:00.000Z');
      }));
  });
  ...
  describe('endOf', () => {
    ...
    it('week', () => {
      ...
      strictEqual(end.getPartUtc('weekDay'), 6);
    });
    ...
    it.effect('returns the zoned end of the month', () =>
      Effect.gen(function* () {
        yield* setTo2024NZ;
        const now = yield* DateTime.nowInCurrentZone.pipe(DateTime.withCurrentZoneNamed('Pacific/Auckland'));
        const future = now.endOf('month');
        strictEqual(future.toDateUtc.toISOString(), '2024-01-31T10:59:59.999Z');
        strictEqual(future.toDate.toISOString(), '2024-01-31T23:59:59.999Z');
      }));
  });
  ...
  describe('startOf', () => {
    it('month', () => {
      const mar = DateTime.makeUnsafe('2024-03-15T12:00:00.000Z');
      const end = mar.startOf('month');
      strictEqual(end.toJSON(), '2024-03-01T00:00:00.000Z');
    });

    it('month duplicated', () => {
      const mar = DateTime.makeUnsafe('2024-03-15T12:00:00.000Z');
      const end = mar.startOf('month').startOf('month');
      strictEqual(end.toJSON(), '2024-03-01T00:00:00.000Z');
    });

    it('feb leap year', () => {
      const feb = DateTime.makeUnsafe('2024-02-15T12:00:00.000Z');
      const end = feb.startOf('month');
      strictEqual(end.toJSON(), '2024-02-01T00:00:00.000Z');
    });

    it('week', () => {
      const start = DateTime.makeUnsafe('2024-03-15T12:00:00.000Z');
      const end = start.startOf('week');
      strictEqual(end.toJSON(), '2024-03-10T00:00:00.000Z');
      strictEqual(end.getPartUtc('weekDay'), 0);
    });
  ...
  describe.skipIf(isDeno)('format zoned', () => {
    ...
    it.effect.skipIf(isDeno)('long with offset', () =>
      Effect.gen(function* () {
        const now = yield* DateTime.now;
        const formatted = now.setZoneOffset(10 * 60 * 60 * 1000).format({
          locale: 'en-US',
          dateStyle: 'long',
          timeStyle: 'short'
        });
  ...
  describe('setPartsUtc', () => {
    it('partial', () => {
      ...
      const updated = date.setPartsUtc({
        year: 2023,
        month: 1
      });
    ...
    it('ignores time zones', () => {
      const date = DateTime.makeUnsafe({
        ...
        month: 12,
        day: 25
      }).setZoneNamedUnsafe('Pacific/Auckland');
      strictEqual(date.toJSON(), '2024-12-25T00:00:00.000Z');
  ...
  describe('formatIso', () => {
    it('full', () => {
      ...
      strictEqual(now.formatIso, '2024-03-15T12:00:00.000Z');
    });
  });
  ...
  describe('formatIsoOffset', () => {
    it.effect('correctly adds offset', () =>
      Effect.gen(function* () {
        const now = yield* DateTime.nowInCurrentZone.pipe(DateTime.withCurrentZoneNamed('Pacific/Auckland'));
        strictEqual(now.formatIsoOffset, '1970-01-01T12:00:00.000+12:00');
      }));
  });
    ...
    it.effect('correctly adds offset', () =>
      Effect.wrap(
        _Effect.provide(
          ...
          DateTime.layerCurrentZoneNamed('Pacific/Auckland')
        )
      ));
  });
  ...
  describe('makeZonedFromString', () => {
    it.effect('parses an instant with an offset and IANA zone', () =>
      Effect.gen(function* () {
        ...
        assertSome(dt.map((value) => value.toJSON()), '2024-07-21T08:12:34.112Z');
      }));
    ...
    it.effect('parses an offset-only string as an Offset zone', () =>
      Effect.gen(function* () {
        ...
        assertSome(dt.map((value) => value.zone._tag), 'Offset');
        assertSome(dt.map((value) => value.toJSON()), '2024-07-21T08:12:34.112Z');
      }));
    ...
    it.effect('parses a UTC offset-only string as an Offset zone', () =>
      Effect.gen(function* () {
        ...
        assertSome(dt.map((value) => value.zone._tag), 'Offset');
        assertSome(dt.map((value) => value.toJSON()), '2024-07-21T20:12:34.112Z');
      }));

    it.effect('roundtrips a bracketed IANA zone string', () =>
      Effect.gen(function* () {
        const dt = DateTime.makeZonedFromString('2024-07-21T20:12:34.112546348+12:00[Pacific/Auckland]').flatMap((d) =>
          DateTime.makeZonedFromString(d.formatIsoZoned)
        );
        assertSome(dt.map((value) => DateTime.zoneToString(value.zone)), 'Pacific/Auckland');
        assertSome(dt.map((value) => value.toJSON()), '2024-07-21T08:12:34.112Z');
      }));
  });
  ...
  describe('core interop', () => {
    it('wrap and the dateTime getter round-trip', () => {
      const coreUtc = _DateTime.makeUnsafe('2024-01-01T12:00:00Z');
      const utc = DateTime.wrap(coreUtc);
      assertTrue(utc.isUtc());
      strictEqual(utc.dateTime, coreUtc);
      strictEqual(utc.formatIso, '2024-01-01T12:00:00.000Z');

      const coreZoned = _DateTime.makeZonedUnsafe('2024-01-01T12:00:00Z', {
        timeZone: 'Pacific/Auckland'
      });
      const zoned = DateTime.wrap(coreZoned);
      assertTrue(zoned.isZoned());
      strictEqual(zoned.dateTime, coreZoned);
    });

    it('is', () => {
      const fluent = DateTime.makeUnsafe('2024-01-01');
      assertTrue(DateTime.is(fluent));
      assertFalse(DateTime.is(fluent.dateTime));
      assertFalse(DateTime.is(null));
    });

    it('with applies a core transformation', () => {
      const next = DateTime.makeUnsafe('2024-01-01').with((core) => _DateTime.add(core, { days: 1 }));
      assertTrue(DateTime.is(next));
      strictEqual(next.formatIso, '2024-01-02T00:00:00.000Z');
    });
  });
});
